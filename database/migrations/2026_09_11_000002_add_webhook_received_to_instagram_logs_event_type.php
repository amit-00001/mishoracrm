...
return new class extends Migration
{
    public function up(): void
    {
        if (Schema::getConnection()->getDriverName() === 'pgsql') {
            ...
            DB::statement("ALTER TABLE instagram_logs ADD CONSTRAINT instagram_logs_event_type_check CHECK (event_type IN ('comment','dm_received','dm_sent','automation_triggered','chatbot_triggered','n8n_triggered','oauth_connect','oauth_deauthorize','data_deletion_request','webhook_received'))");
        } else {
    ...
    public function down(): void
    {
        if (Schema::getConnection()->getDriverName() === 'pgsql') {
            DB::statement('ALTER TABLE instagram_logs DROP CONSTRAINT IF EXISTS instagram_logs_event_type_check');
            DB::statement("ALTER TABLE instagram_logs ADD CONSTRAINT instagram_logs_event_type_check CHECK (event_type IN ('comment','dm_received','dm_sent','automation_triggered','chatbot_triggered','n8n_triggered','oauth_connect','oauth_deauthorize','data_deletion_request'))");
        } else {
